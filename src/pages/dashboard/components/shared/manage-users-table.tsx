...
const ManageUsersTable: FC<IManageUsersTableProps> = ({ showBalance, users, isLoading }) => {
    ...
    return (
        <div style={{ minWidth: "100%", display: "table" }} className="card-white overflow-hidden">
            <div className="relative w-full max-h-[calc(100vh-310px)] overflow-auto">
                {
                    isLoading ?
                        <Loading className='h-[calc(100vh-310px)]' />
                        :
                            ...
                            <TableBody>
                                {users?.map((user) => (
                                    <TableRow key={user._id} className="border-b border-black/10 hover:bg-gray-50">
                                        ...
                                        <TableCell className="p-4">
                                            <UpdateUserStatus status={user.status} userId={user._id} />
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
